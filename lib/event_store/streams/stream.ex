defmodule EventStore.Streams.Stream do
  ...
  # Validates expected versions for all appends, tracking running versions
  # so the same stream can appear multiple times in a batch.
  #
  # First occurrence: checked against the database.
  # Subsequent occurrences: checked against the running version
  # (previous expected_version + events appended so far for that stream).
  defp validate_expected_versions(conn, appends, opts) do
    Enum.reduce_while(appends, %{}, fn append, running_versions ->
      ...
      case Map.fetch(running_versions, stream_uuid) do
        :error ->
          # First time seeing this stream — validate against the database
        ...
        {:ok, running_version} ->
          # Seen before — validate against the running version
  ...
  defp validate_first_occurrence(
    ...
    if expected_version == :any_version do
      # Need the current version from DB to track correctly
